...
try {
	...
	# get the router
	$Router = new UltraMVC\Routers\Router($Bootstrap);
	# get the controller
	$Router->callController();

} catch (\UltraMVC\Routers\RouterException $RouterException) {
	# if there was an error in the router - send a message
	$Error = new \UltraMVC\Views\Errors\Error;
	$Error->printError($RouterException->getMessage());

} catch (\UltraMVC\Responses\HttpResponse $HttpResponse) {
	...
	if (function_exists('lastExceptionHandle')) {
		...
		if (SHOW_HTML_ON_LAST_EXCEPTION) {
			print '
				...
				<html>
					<head>
						<title>UltraMVC Exception Error</title>
						<!-- 
							"UltraMVC" javascript and css gets rerouted to library media files
							UltraMVC css takes advantage of less and bootstrap.  For more see
							Lesscss.org and the bootstrap repo at: 
						-->
						<link href="UltraMVC/css/Ultra-MVC.css" />
					</head>
					<body>
						<div id="Ultra-MVC-enc">
